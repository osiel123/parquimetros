...
//Rare exceptions to indicate a bug in the _parquimetros code (Eg, method that a subclass should have implemented)
...
+ (id)createProxy:(NSString*)qualifiedName withProperties:(NSDictionary*)properties inContext:(id<TiEvaluator>)context
{
	...
	if (proxyClass == nil) {
		NSString *_parquimetros = [NSString stringWithFormat:@"%@%s",@"Ti","tanium."];
		if ([qualifiedName hasPrefix:_parquimetros]) {
		...
		if (proxyClass==nil) {
			...
			@throw [NSException exceptionWithName:@"org.parquimetros.module"
